...
def get_access_copy_actions(doi_parts, primary_type):
    data = {
        'target_server': '',
        'target_path': '',
        'thumbnail_path': '',
        ...
        if matches_any_pattern(digital_object_id):
            ...
            if len(parts) == 3:
                try:
                    ...
                    return Response({'error': 'No container exists with this barcode'},
                                    status=404)
            ...
            primary_type = finding_aids_entity.primary_type.type if finding_aids_entity else 'Moving Image'

            if archival_unit and level:
                return Response({
                    'doi': doi,
                    'container_reference_code': f'{archival_unit.reference_code}:{container.container_no}',
                    'fa_entity_reference_code': finding_aids_entity.archival_reference_code if finding_aids_entity else 'N/A',
                    'primary_type': primary_type,
                    'level': level,
                    'access_copy_to_catalog': get_access_copy_actions(parts, primary_type)
                }, status=200)
...
class DigitalObjectUpsert(APIView):
    ...
    def post(self, request, level, digital_object_id):
        if matches_any_pattern(digital_object_id):
            return Response({'info': 'OK'}, status=200)
        else:
            return Response({'error': 'Invalid digital object ID'}, status=400)