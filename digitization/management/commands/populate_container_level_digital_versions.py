...
class Command(BaseCommand):
    def handle(self, *args, **options):
        containers = list(self.get_target_containers())
        self.create_access_copies(containers)
        self.create_masters(containers)

    def get_target_containers(self):
        return Container.objects.filter(
            digital_version_exists=True,
            digital_versions__isnull=True
        ).exclude(
    ...
    def create_masters(self, containers):
    ...
    def create_access_copies(self, containers):
        for container in containers:
            identifier = container.barcode
